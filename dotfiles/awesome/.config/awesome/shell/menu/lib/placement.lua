...
local function get_theme_props()
	local theme = get_menu_theme()
...
function P.total_height(item_count)
	local height = P.item_height()
	return height * (item_count or 0)
end

function P.coords_for_tabs(s, anchor_left_x, item_count)
	local total_height = P.total_height(item_count)
