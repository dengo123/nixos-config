# modules/home/services/autorandr/default.nix
{
  config, lib, namespace, pkgs, ...
}:
with lib;
with lib.${namespace}; let
  cfg = config.${namespace}.services.autorandr;
in {
  options.${namespace}.services.autorandr =
    { enable = mkEnableOption "Enable autorandr for automatic monitor profile switching (X11 only)"; };

  config = mkIf cfg.enable {
    programs.autorandr = {
      enable = true;

      profiles = {
        dual = {
          fingerprint = {
            "DP-2" =
              "";
            "DP-4" =
              "";
          };
          config = {
            "DP-2" = {
              enable = true; mode = "1920x1080"; rate = "60.00"; position = "0x0"; rotate = "left";
            };
            "DP-4" = {
              enable = true; primary = true; mode = "1920x1080"; rate = "60.00"; position = "1080x420";
            };
            "HDMI-0" = { enable = false; };
            "DP-0" = { enable = false; }; "DP-1" = { enable = false; };
            "DP-3" = { enable = false; }; "DP-5" = { enable = false; };
          };
        };

        # Spiegel-Profil: DP-4 ⇄ HDMI-0 (1080p@60), echtes Klonen
        tv-mirror = {
          fingerprint = {
            "HDMI-0" =
              "";
            "DP-4" =
              "";
            "DP-2" =
              "";
          };
          config = {
            "DP-4"   = { enable = true; primary = true; mode = "1920x1080"; rate = "60.00"; position = "1080x420"; };
            "HDMI-0" = { enable = true;              mode = "1920x1080"; rate = "60.00"; position = "1080x420"; };
            "DP-2" = {
              enable = true; mode = "1920x1080"; rate = "60.00"; position = "0x0"; rotate = "left";
            };
            "DP-0" = { enable = false; };
            "DP-1" = { enable = false; }; "DP-3" = { enable = false; };
            "DP-5" = { enable = false; };
          };
        };
      };

      hooks.postswitch = {
        notify = ''notify-send -a autorandr "Display profile switched" "Now using $AUTORANDR_CURRENT_PROFILE"'';
        tv_mirror_sameas = ''
          if [ "$AUTORANDR_CURRENT_PROFILE" = "tv-mirror" ]; then
            # Reset evtl. Reste
            xrandr --output HDMI-0 --transform none --scale 1x1
            xrandr --output DP-4   --transform none --scale 1x1
            # identische Modes sicherstellen
            xrandr --output DP-4   --mode 1920x1080 --rate 60
            xrandr --output HDMI-0 --mode 1920x1080 --rate 60
            # echtes Spiegeln
            xrandr --output HDMI-0 --same-as DP-4
          fi
        '';
      };
    };

    # (Optional) Tools parat haben
    home.packages = [ pkgs.xorg.xrandr ];
  };
}
