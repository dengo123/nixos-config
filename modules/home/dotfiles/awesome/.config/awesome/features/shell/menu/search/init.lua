...
local function resolve_theme(shared, mod_tbl, overrides)
	...
	-- akzeptiere sowohl search_* als auch generische Namen
	local t = {
		...
		height = pick(shared.search_height, mod_tbl.height, 36),
	...
	-- fallback: expand_w mindestens hit_w
...
--   width_ctl (REQUIRED), height_ctl?,
--   shared_theme?, search_overrides?, history_path?, on_search?
function M.build(opts)
	...
	local function apply_collapsed_style()
		...
		bg_box.bg = "#00000000"
		bg_box.fg = T.fg
		width_ctl.width = T.hit_w
		bg_box.forced_width = T.hit_w
	end
	...
	local function apply_expanded_style()
		...
		bg_box.bg = T.bg
		bg_box.fg = T.fg
		width_ctl.width = T.expand_w
		bg_box.forced_width = T.expand_w
	end

	local function cancel_search()
		if prompt_node and prompt_node.set_visible then
