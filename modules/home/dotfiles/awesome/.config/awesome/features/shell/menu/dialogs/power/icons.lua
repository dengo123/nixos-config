...
-- ===== Lokale Utilities (aus dem alten helpers ausgelagert) =====
...
-- ==== Öffentliche API von icons.lua =================================

function Icons.compute_metrics(th, dialog_w, dialog_h)
	-- Direkter Aufruf der lokalen Variante
	return compute_icon_metrics(th, dialog_w, dialog_h)
end

-- Baut eine Icon-Action-Reihe (Buttons gleichmäßig verteilt)
-- actions: { {icon=..., emoji=..., emoji_font=..., label=..., on_press=function(close_fn) end, autoclose=true?}, ... }
		...
		local btn = W.mk_icon_button({
			...
			on_press = function()
				...
				if a.on_press then
					...
					-- datengetriebener Fallback via Lib.actions.run (wenn dein mk_icon_button das intern nutzt)
