...
function Icons.actions_row(actions, th, geom, get_close_ref)
  ...
  local row = build_even_row(
    cells,
    targets,
    geom.icon_cell_w or ((geom.icon_size or 64) + 24),
    geom.place_w or 400
  )
  ...
  for i, a in ipairs(actions) do
    local btn = W.mk_icon_button({
      ...
      on_press   = function()
        local close_fn
        if type(get_close_ref) == "function" then
          local ok, cf = pcall(get_close_ref); if ok then close_fn = cf end
        end
        if a.on_press then
          a.on_press(close_fn or function() end)
        elseif a.autoclose and close_fn then
