...
local function move_tag_to_screen(dir)
	...
	t.screen = target
	t:view_only()
	-- Fokus folgt (optional, hier aktiv):
...
local function move_client_to_screen(dir)
	...
	-- packe das Fenster in den aktuell aktiven Tag des Ziel‑Screens
...
local function move_client_to_neighbor_tag(delta, follow)
	...
	if c and nt then
		c:move_to_tag(nt)
		if follow then
			nt:view_only() -- zum Ziel‑Tag wechseln
			client.focus = c -- Fokus auf das verschobene Fenster setzen
			c:raise()
		end
	end
end
...
local globalkeys = gears.table.join(
	...
	-- Fenster zwischen Tags (Mod+Ctrl+Shift) -> Fokus folgt
	...
	-- Fenster auf anderen Monitor (Mod+Shift+Alt)
	...
	-- Tag zwischen Monitoren verschieben (Mod+Ctrl+Alt)
